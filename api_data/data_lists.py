...
def make_url(api_key, api_token):
...
def get_list(id_list, api_key, api_token):
    ...
    response = requests.get(url=make_url(api_key, api_token), timeout=10)
    return response


def update_list(id_list, data, api_key, api_token):
    """Update a list from the API"""
    request = requests.put(url=make_url(api_key, api_token), data=data, timeout=10)
    return request


def create_list(data, api_key, api_token):
    """Create a list from the API"""
    request = requests.post(url=make_url(api_key, api_token), data=data, timeout=10)
    return request


def archive_a_list(id_list, value, api_key, api_token):
    """Archive a list from the API"""
    request = requests.put(url=make_url(api_key, api_token), params=value, timeout=10)
    return request
